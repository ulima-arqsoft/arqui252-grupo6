> [4. Decisiones Iniciales de Arquitectura](../4.md) › [4.2. Módulo 2 / Integrante 2](4.2.md)

# 4.2. Módulo 2 / Integrante 2

## 🧠 Decisión 1: Modelo de Coordinación

### Título
**Elección entre Comunicación en Tiempo Real mediante WebSockets vs Peticiones REST Tradicionales para la Coordinación de Colaboradores**

### Contexto
El Módulo 2 — Colaboración de Ideas gestiona la interacción entre usuarios dentro de la plataforma NEXUS, permitiendo que diferentes colaboradores trabajen en conjunto mediante canales de chat, subgrupos y notificaciones en tiempo real.  
Estas funcionalidades (HU019–HU021) requieren comunicación inmediata y bidireccional para reflejar mensajes y cambios en todos los dispositivos de los participantes.

El backend se desarrolla en **Node.js con Express**, y el frontend en **React**, ambos compatibles con **REST API** y **WebSockets (Socket.IO)**, lo que plantea la necesidad de definir el modelo de coordinación principal.

### Alternativas
1. **REST API Tradicional (HTTP)**  
   - Comunicación basada en solicitudes cliente-servidor independientes.  
   - Cada acción (enviar mensaje, aceptar solicitud, etc.) requiere una llamada HTTP.  
   - Fácil de implementar, pero con mayor latencia.  
   - No ofrece comunicación bidireccional.

2. **Comunicación en Tiempo Real mediante WebSockets (Socket.IO)**  
   - Conexión persistente cliente-servidor.  
   - Comunicación bidireccional ideal para mensajería y notificaciones.  
   - Baja latencia y transmisión inmediata de eventos.  
   - Integración nativa con Express y React.

### Criterios de Elección
- **Latencia y fluidez**: comunicación instantánea.  
- **Escalabilidad**: múltiples sesiones simultáneas.  
- **Compatibilidad técnica**: integración con Node.js y React.  
- **Experiencia de usuario**: interacción natural, sin recargas.  
- **Mantenibilidad**: extensión futura hacia notificaciones o videollamadas.

### Decisión
Se implementará **WebSockets (Socket.IO)** para habilitar comunicación en tiempo real entre los colaboradores dentro del módulo.

### Sustento
- Permite mensajes instantáneos (HU019) y notificaciones inmediatas (HU018).  
- Soporta reconexión automática y eventos personalizados (ESC-21).  
- Integra sin fricción con Express y React, conservando la arquitectura modular.  
- Baja latencia promedio (<200 ms).

### Conclusión Técnica
**Stack:** Node.js + Express + Socket.IO  
**Atributos reforzados:** Disponibilidad, Interoperabilidad, Usabilidad y Rendimiento.  

---

## ⚙️ Decisión 2: Estrategia de Persistencia de Mensajes

### Título
**Elección entre Almacenamiento Temporal en Memoria (Redis) vs Almacenamiento Permanente en Base de Datos (SQL Server)**

### Contexto
El Módulo 2 requiere guardar los mensajes enviados entre colaboradores para permitir su recuperación en caso de desconexión (ESC-21) y mantener trazabilidad del historial (ESC-28).  
Se evaluó si los mensajes deben almacenarse temporalmente en memoria o persistirse directamente en la base de datos principal.

### Alternativas
1. **Almacenamiento Temporal en Memoria (Redis)**  
   - Alta velocidad de lectura/escritura.  
   - Ideal para colas de mensajes activos y sesiones abiertas.  
   - No garantiza persistencia a largo plazo.  

2. **Almacenamiento Permanente en SQL Server**  
   - Registro duradero de todas las conversaciones.  
   - Mayor latencia de escritura.  
   - Escalabilidad limitada bajo alta concurrencia.

### Criterios de Elección
- **Rendimiento:** velocidad en la entrega de mensajes activos.  
- **Trazabilidad:** necesidad de guardar el historial del chat.  
- **Disponibilidad:** evitar pérdida de mensajes durante fallas o reconexiones.  
- **Costo de mantenimiento:** simplicidad de integración con el backend existente.

### Decisión
Se adopta una **estrategia híbrida**:  
- Redis para mensajes activos en tiempo real.  
- SQL Server para almacenamiento histórico mediante sincronización periódica.

### Sustento
- Redis reduce la latencia de envío/recepción (ESC-24).  
- SQL Server conserva los mensajes de manera trazable (ESC-28).  
- En caso de falla, los mensajes pendientes en Redis se sincronizan automáticamente.

### Conclusión Técnica
**Stack:** Redis + SQL Server  
**Atributos reforzados:** Rendimiento, Trazabilidad y Disponibilidad.  

---

## 🔐 Decisión 3: Control de Acceso y Roles

### Título
**Implementación de Autenticación JWT con Middleware de Roles vs Sesiones de Usuario Tradicionales**

### Contexto
Los colaboradores deben acceder únicamente a los chats en los que participan.  
El sistema necesita validar roles (creador, colaborador, moderador) y proteger la comunicación contra accesos no autorizados (ESC-25, ESC-29).

### Alternativas
1. **Sesiones de Usuario Tradicionales (Cookies/Session Storage)**  
   - Gestión de sesión en servidor.  
   - Requiere sincronización compleja en múltiples instancias.  
   - Escalabilidad limitada.  

2. **Autenticación JWT con Middleware de Roles**  
   - Tokens firmados enviados en cada petición WebSocket o REST.  
   - Validación descentralizada sin estado (stateless).  
   - Escalable y compatible con microservicios.

### Criterios de Elección
- **Seguridad:** validación robusta sin exponer credenciales.  
- **Escalabilidad:** sesiones distribuidas entre múltiples nodos.  
- **Mantenibilidad:** fácil de extender a nuevos tipos de rol.  
- **Compatibilidad:** integración con middleware de Express y Socket.IO.

### Decisión
Implementar **autenticación basada en JWT** y **middleware de roles personalizados** para cada canal de colaboración.

### Sustento
- Permite identificar usuarios y roles sin mantener sesiones persistentes.  
- Controla permisos por canal y acción (HU022, ESC-25).  
- Facilita la revocación inmediata al abandonar el proyecto (ESC-29).  
- Cumple con los requisitos de seguridad y escalabilidad distribuidas.

### Conclusión Técnica
**Stack:** Express + JWT + Middleware de Roles  
**Atributos reforzados:** Seguridad, Escalabilidad y Mantenibilidad.  

---

## 🧩 Decisión 4: Sistema de Notificaciones Inteligentes

### Título
**Elección entre Notificaciones Push en Tiempo Real vs Notificaciones Agrupadas por Prioridad**

### Contexto
Los colaboradores activos en múltiples proyectos reciben notificaciones de nuevos mensajes, menciones y solicitudes (HU018, ESC-26).  
Un exceso de alertas simultáneas puede generar saturación de la interfaz y disminuir la usabilidad.

### Alternativas
1. **Notificaciones Push Inmediatas**  
   - Cada evento genera una alerta instantánea.  
   - Alta inmediatez pero puede saturar la experiencia.  

2. **Notificaciones Agrupadas por Prioridad (Batch)**  
   - Se agrupan mensajes y eventos de baja prioridad.  
   - Mejora la experiencia, reduce ruido informativo.  
   - Requiere un sistema de clasificación y almacenamiento temporal.

### Criterios de Elección
- **Usabilidad:** reducir interrupciones y distracciones.  
- **Rendimiento:** disminuir el número de eventos simultáneos.  
- **Escalabilidad:** manejar notificaciones de cientos de chats activos.  

### Decisión
Adoptar **notificaciones agrupadas por prioridad**, enviadas en lotes según relevancia (menciones directas, invitaciones o mensajes generales).

### Sustento
- Reduce el ruido informativo (ESC-26).  
- Optimiza recursos del servidor y mejora la experiencia.  
- Permite personalizar alertas por canal o tipo de mensaje.  

### Conclusión Técnica
**Stack:** Redis Streams + Socket.IO Notifications  
**Atributos reforzados:** Usabilidad, Rendimiento y Escalabilidad.  

---

## 🧩 Decisión 5: Escalabilidad Horizontal del Chat

### Título
**Elección entre Servidor Único Centralizado vs Escalado Horizontal con Balanceador de Carga**

### Contexto
El módulo debe soportar miles de usuarios concurrentes (ESC-30).  
Un servidor único puede limitar la capacidad de respuesta y provocar caídas bajo alta demanda.

### Alternativas
1. **Servidor Único Centralizado**  
   - Simplicidad de implementación.  
   - Escalabilidad limitada y riesgo de sobrecarga.  

2. **Escalado Horizontal con Balanceador de Carga (Nginx/PM2 Cluster)**  
   - Distribuye las conexiones WebSocket entre múltiples instancias.  
   - Mejora la disponibilidad y tolerancia a fallos.  
   - Requiere una configuración inicial más compleja.

### Criterios de Elección
- **Escalabilidad:** manejar picos de usuarios concurrentes.  
- **Disponibilidad:** evitar caídas del servicio.  
- **Rendimiento:** mantener baja latencia en todos los nodos.  

### Decisión
Implementar **escalado horizontal con balanceador de carga Nginx** y **módulo de clustering de Node.js (PM2)**.

### Sustento
- Garantiza alta disponibilidad durante picos (ESC-30).  
- Distribuye la carga equitativamente entre instancias.  
- Facilita el mantenimiento y despliegue sin downtime.  

### Conclusión Técnica
**Stack:** Node.js Cluster + Nginx + Redis Adapter  
**Atributos reforzados:** Escalabilidad, Disponibilidad y Rendimiento. 

[⬅️ Anterior](../4.1/4.1.md) | [🏠 Home](../../README.md) | [Siguiente ➡️](../4.3/4.3.md)



