...
export const useNexusController = () => {
  ...
  // --- OFFLINE SYNC HOOK ---
  const offlineSync = useOfflineSync(currentUser, fetchIdeas);
  ...
  // --- PUBLICAR IDEA CON SOPORTE OFFLINE ---
  const publishIdea = async (formData: any) => {
    const originalPublish = async (data: any) => {
      setLoading(true);
      try {
        const response = await fetch(`${API_URL}/ideas`, {
          method: 'POST',
    ...
    const result = await offlineSync.publishIdeaOffline(formData, originalPublish);
    if (result.offline) {
      setShowPublishForm(false);
    }
  };
  ...
  return {
    state: {
      ...
      pendingIdeasCount: offlineSync.pendingIdeasCount,
